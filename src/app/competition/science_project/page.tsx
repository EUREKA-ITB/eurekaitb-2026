...
type Hadiah = { juara1: string; juara2: string; juara3: string; };
...
export default function ScienceProjectPage() {
  ...
  const info = { 
    type: "Kelompok (2 - 3 Orang)", 
    desc: "Science Project Competition EUREKA! ITB 2026 adalah kompetisi inovasi sains tingkat nasional bagi pelajar SMA/Sederajat. Dirancang untuk peserta mengembangkan ide dan inovasi berbasis sains secara tim." 
  };

  const documents = [
    { id: "guidebook", title: "Guidebook Resmi", url: "/guidebooks/science_project.pdf" },
    { id: "format_laporan", title: "Format Laporan", url: "/guidebooks/format_laporan_sp.pdf" },
  ];
  ...
  const timelineData = [
    { date: "16 - 29 Agustus 2026", title: "Registrasi Gelombang 1", desc: "Pendaftaran peserta dan pengumpulan abstrak secara online" },
    { date: "30 Agustus 2026 - 19 September 2026", title: "Registrasi Gelombang 2", desc: "Pendaftaran peserta dan pengumpulan abstrak secara online" },
    { date: "20 - 30 September 2026", title: "Registrasi Gelombang 3", desc: "Pendaftaran peserta dan pengumpulan abstrak secara online" },
    { date: "31 Oktober 2026", title: "Babak Semifinal", desc: "Peserta mengumpulkan full paper secara online" },
    { date: "28 - 29 November 2026", title: "Babak Final", desc: "Dilaksanakan secara luring di Kampus ITB Ganesha" },
  ];

  const faqData = [
    { q: "Apakah peserta yang lolos ke babak final wajib hadir langsung di ITB?", a: "Ya, untuk babak final diwajibkan hadir secara luring di Kampus ITB Ganesha." },
    { q: "Bagaimana jika secara mendadak berhalangan ikut final?", a: "Konfirmasi peserta maksimal dilakukan sebelum tahap technical meeting finalis." },
    { q: "Bolehkah mengganti anggota kelompok di tengah kompetisi?", a: "Pergantian anggota maksimal dilakukan H-7 sebelum tahap final dengan menyertakan surat keterangan resmi." },
  ];

  const hadiah = { juara1: "Rp5.000.000", juara2: "Rp 3.500.000", juara3: "Rp 2.000.000" };
  ...
  return (
    <div className="min-h-screen bg-blue-marine text-white font-sans overflow-x-hidden pt-24 pb-20">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 w-full flex flex-col gap-16">
        ...
        <section className="relative z-10">
          <div className="bg-white/5 border border-white/10 p-6 md:p-10 rounded-[2rem] backdrop-blur-md">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
              <div>
                <h2 className="font-display text-2xl font-bold flex items-center gap-3 mb-2"><BookOpen className="text-sunlight-orange" /> Dokumen & Panduan</h2>
            ...
            <div className="w-full bg-[#0a0f24] border border-white/10 rounded-2xl overflow-hidden relative h-[400px] md:h-[700px] flex flex-col items-center justify-center group">
              <object data={`${activeDoc.url}#toolbar=0`} type="application/pdf" className="hidden md:block w-full h-full absolute inset-0 z-10">
                <div className="flex flex-col items-center justify-center h-full p-8 text-center bg-black/40">
                  ...
                  <p className="text-sm text-silver-shine">Pastikan file <b>{activeDoc.url}</b> sudah di-upload di folder public.</p>
                </div>
              </object>
              <div className="md:hidden flex flex-col items-center justify-center p-8 text-center h-full relative z-20">
                ...
                <p className="text-silver-shine text-sm mb-6">Perangkat mobile tidak mendukung pratinjau PDF langsung.</p>
                <a href={activeDoc.url} target="_blank" rel="noreferrer" className="bg-sunlight-orange text-blue-marine font-bold px-6 py-3 rounded-xl shadow-lg hover:bg-yellow-400 transition-colors">Buka / Unduh {activeDoc.title}</a>
              </div>
              <a href={activeDoc.url} download className="hidden md:flex absolute top-4 right-6 z-20 bg-black/50 hover:bg-black/80 backdrop-blur-md border border-white/20 px-4 py-2 rounded-lg text-white text-xs font-bold items-center gap-2 transition-colors">Unduh PDF</a>
            </div>
          </div>
        </section>
                ...
                <div key={index} className={`relative flex flex-col md:flex-row items-start md:items-center ${index % 2 === 0 ? "md:flex-row-reverse" : ""}`}>
                  ...
                  <div className={`ml-16 md:ml-0 md:w-1/2 ${index % 2 === 0 ? "md:pl-12" : "md:pr-12 md:text-right"}`}>
                    <div className="bg-white/5 border border-white/10 p-5 rounded-2xl backdrop-blur-sm hover:border-sunlight-orange/30 transition-colors">
                      <span className="inline-block px-3 py-1 bg-sunlight-orange/10 text-sunlight-orange text-xs font-bold rounded-full mb-3">{item.date}</span>
        ...
        <section className="relative z-10 py-10">
          <div className="text-center mb-12"><h2 className="font-display text-3xl font-bold flex items-center justify-center gap-3"><Medal className="text-sunlight-orange" /> Hadiah Pemenang</h2></div>
          <div className="flex flex-col md:flex-row items-center justify-center gap-6 max-w-4xl mx-auto">
            <div className="order-2 md:order-1 bg-white/5 border border-white/10 p-6 rounded-3xl text-center w-full md:w-1/3 hover:-translate-y-2 transition-transform">
              ...
              <h3 className="text-2xl font-bold text-white">{hadiah.juara2}</h3>
            </div>
            <div className="order-1 md:order-2 bg-gradient-to-b from-sunlight-orange/20 to-transparent border border-sunlight-orange/50 p-8 rounded-[2.5rem] text-center w-full md:w-1/3 shadow-[0_0_30px_rgba(255,184,0,0.15)] transform md:-translate-y-4">
              ...
              <h3 className="text-3xl font-bold text-white">{hadiah.juara1}</h3>
            </div>
            <div className="order-3 md:order-3 bg-white/5 border border-white/10 p-6 rounded-3xl text-center w-full md:w-1/3 hover:-translate-y-2 transition-transform">
              ...
              <h3 className="text-2xl font-bold text-white">{hadiah.juara3}</h3>
            </div>
          </div>
          <p className="text-center text-xs text-silver-shine mt-8 max-w-2xl mx-auto">*Seluruh peserta yang mengikuti babak penyisihan dan semifinal akan menerima e-sertifikat, sementara peserta yang berhasil lolos ke babak final akan memperoleh sertifikat fisik.</p>
        </section>
        ...
        <section className="relative z-10 py-10 border-t border-white/10">
          <div className="flex flex-col md:flex-row gap-10">
            <div className="md:w-1/3">
              ...
              <p className="text-sm text-silver-shine leading-relaxed">Berikut adalah kumpulan pertanyaan yang paling sering muncul terkait <b>{formatName}</b>.</p>
            </div>
