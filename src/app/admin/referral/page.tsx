...
export default function AdminReferralPage() {
  const [codes, setCodes] = useState<ReferralData[]>([]);
  ...
  const handleGenerate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        setPartnerName("");
        setTier("Quantum");
        const freshRes = await fetch("/api/referral");
        if (freshRes.ok) {
          const data: ReferralData[] = await freshRes.json();
          setCodes(data);
        }
      } else {
          ...
          <div className="lg:col-span-2">
            <div className="bg-white/5 border border-white/10 rounded-3xl backdrop-blur-sm overflow-hidden shadow-2xl">
              ...
              <div className="overflow-x-auto">
                {isLoading ? (
                  <div className="p-16 flex justify-center items-center text-silver-shine">
                    <Loader2 size={32} className="animate-spin text-sunlight-orange" />
                  </div>
                ) : codes.length === 0 ? (
