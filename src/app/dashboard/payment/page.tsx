...
import { AlertCircle, CheckCircle2, Clock, Copy, Lock, ShieldCheck, Download } from "lucide-react";
...
export default async function PaymentPage() {
  ...
  const phase = "Gelombang 1";
  const basePrice = 75000;
  const uniqueCode = parseInt((leader?.phoneNumber || "000").slice(-3)) || 0; 
  const totalPayment = basePrice + uniqueCode;
  const formatRp = (nominal: number) => new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", minimumFractionDigits: 0 }).format(nominal);
  ...
  return (
    <div className="min-h-screen bg-blue-marine text-white font-sans p-4 sm:p-8 md:p-12 box-border overflow-x-hidden">
      <div className="max-w-5xl mx-auto w-full pt-20">
        ...
        {isVerified ? (
          /* ========================================= */
          /* TAMPILAN INVOICE LUNAS (PRINTABLE)        */
          /* ========================================= */
            ...
            <div className="relative z-10">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-gray-200 pb-6 mb-8 gap-4">
                ...
                <div className="bg-green-100 text-green-700 px-4 py-2 rounded-lg flex items-center gap-2 font-bold text-sm">
                  <CheckCircle2 size={18} /> LUNAS / VERIFIED
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-8">
                <div>
                  <p className="text-xs text-gray-400 uppercase tracking-wider mb-1">Diterima Dari:</p>
                ...
                <div className="sm:text-right">
                  <p className="text-xs text-gray-400 uppercase tracking-wider mb-1">ID Referensi:</p>
              ...
              <div className="bg-gray-50 rounded-xl p-6 border border-gray-100 mb-8">
                <div className="flex justify-between items-center border-b border-gray-200 pb-4 mb-4">
                  <span className="font-semibold text-gray-700">Kategori Lomba</span>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-display font-bold text-2xl text-sunlight-orange">{formatRp(totalPayment)}</span>
                </div>
              </div>

              <div className="text-center text-xs text-gray-400 mt-12 print:hidden">
                <p>Simpan atau cetak halaman ini sebagai bukti pembayaran yang sah.</p>
              </div>
            </div>
          </div>
        ) : (
          /* ========================================= */
          /* TAMPILAN PEMBAYARAN (UNPAID / PENDING)    */
          /* ========================================= */
          <>
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4 border-b border-white/10 pb-6">
              <div>
                ...
                <h1 className="font-display text-3xl sm:text-4xl font-bold text-white mb-2">Administrasi Tim</h1>
                <p className="text-silver-shine text-sm max-w-lg">
                  Selesaikan kewajiban administrasi untuk memvalidasi kepesertaan tim <span className="font-bold text-white">{userTeam[0].teamName}</span>.
                </p>
              </div>
            </div>
            ...
            {isPending ? (
              <div className="bg-white/5 border border-sunlight-orange/50 rounded-3xl p-8 sm:p-12 backdrop-blur-sm text-center max-w-2xl mx-auto shadow-[0_0_30px_rgba(255,183,3,0.1)]">
                ...
                <h2 className="font-display text-2xl font-bold text-white mb-2">Menunggu Verifikasi Bendahara</h2>
                <p className="text-silver-shine text-sm mb-8 leading-relaxed">
                  Bukti pembayaran Anda telah kami terima dan sedang dalam antrean pengecekan mutasi bank. Proses ini memakan waktu maksimal 2x24 Jam kerja.
                </p>
                <div className="bg-black/30 rounded-xl p-4 inline-block border border-white/10 mb-6 text-left">
                  <p className="text-xs text-silver-shine mb-1">Nomor Referensi Pendaftaran:</p>
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="bg-white/5 border border-white/10 rounded-3xl p-6 sm:p-8 backdrop-blur-sm relative overflow-hidden h-max shadow-2xl">
                    ...
                    <h2 className="font-display text-xl font-bold mb-6 border-b border-white/10 pb-4">Rincian Tagihan Resmi</h2>
                    
                    <div className="space-y-4 mb-8 text-sm">
                      <div className="flex justify-between"><span className="text-silver-shine">Kategori Lomba</span><span className="font-semibold capitalize">{userTeam[0].compeType.replace("_", " ")}</span></div>
                      <div className="flex justify-between"><span className="text-silver-shine">Harga Tiket ({phase})</span><span className="font-semibold">{formatRp(basePrice)}</span></div>
                      <div className="flex justify-between items-center"><span className="text-silver-shine">Kode Unik Verifikasi</span><span className="font-semibold text-sunlight-orange font-mono bg-sunlight-orange/10 px-2 py-0.5 rounded">+{uniqueCode}</span></div>
                    </div>
                    ...
                    <div className="bg-gradient-to-r from-blue-marine to-black/40 p-5 rounded-xl border border-white/10 text-center mb-6 shadow-inner">
                      <p className="text-xs text-silver-shine mb-1">Total Wajib Transfer:</p>
                      <p className="text-4xl font-display font-bold text-sunlight-orange tracking-tight">{formatRp(totalPayment)}</p>
                    </div>

                    <div className="bg-red-500/10 p-4 rounded-xl border border-red-500/30 mb-6 flex gap-3 items-start">
                      <AlertCircle size={20} className="shrink-0 text-red-400 mt-0.5" />
                      <div className="text-xs text-red-100/80 leading-relaxed">
                        <span className="font-bold text-red-400 block mb-1">Peringatan Anti-Fraud!</span>
                        Transfer dengan nominal <span className="font-bold underline text-white">sangat presisi hingga 3 digit terakhir</span> agar diverifikasi otomatis.
                      </div>
                    </div>
                    
                    <div>
                      <p className="text-xs text-silver-shine mb-2 font-bold uppercase tracking-wider">Rekening Bank Tujuan:</p>
                      <div className="flex justify-between items-center bg-white/10 p-4 rounded-xl border border-white/20 shadow-lg">
                        <div className="flex items-center gap-4">
                          <div className="bg-blue-600 text-white font-black italic text-lg px-2 py-1 rounded">BCA</div>
                          <div>
                            <p className="font-bold tracking-widest text-lg font-mono text-white">1234 567 890</p>
                            <p className="text-xs text-silver-shine">a.n. <span className="text-white font-semibold">EUREKA ITB</span></p>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="bg-white/5 border border-white/10 rounded-3xl p-6 sm:p-8 backdrop-blur-sm flex flex-col shadow-2xl">
                     <div className="flex items-center justify-between border-b border-white/10 pb-4 mb-6">
                        <h2 className="font-display text-xl font-bold">Konfirmasi Transfer</h2>
                        <Lock size={18} className="text-green-400" />
                     </div>
                     <p className="text-silver-shine text-sm mb-6">Unggah struk mutasi yang jelas (resolusi tinggi) agar terbaca oleh sistem.</p>
