...
type UserData = InferSelectModel<typeof users>;
type Props = {
  userData: UserData;
...
export default function SettingsClient({ userData, isGoogleUser, activeTab, sessionImage }: Props) {
  return (
                ...
                <>
                  <div className="flex items-center gap-6 mb-10 pb-10 border-b border-white/10">
                    <div className="relative group cursor-pointer">
                      {userData.image || sessionImage ? (
                        <Image src={userData.image || sessionImage || ""} alt="Profile" width={96} height={96} unoptimized className="w-24 h-24 rounded-full border-2 border-white/20 object-cover group-hover:opacity-50 transition-opacity" />
                      ) : (
                  ...
                  <form className="flex flex-col gap-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-2 pt-6 border-t border-white/10">
                      <div className="flex flex-col gap-2 md:col-span-2">
                        ...
                        <input type="text" defaultValue={userData.institution || ""} placeholder="e.g. Institut Teknologi Bandung" className="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-sunlight-orange" />
                      </div>
                      <div className="flex flex-col gap-2">
                        ...
                        <input type="text" defaultValue={userData.identityNumber || ""} placeholder="NISN or NIM" className="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-sunlight-orange" />
                      </div>
                      <div className="flex flex-col gap-2">
                        ...
                        <select defaultValue={userData.educationLevel || "S1"} className="w-full bg-[#0a102b] border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-sunlight-orange appearance-none">
                ...
                <>
                  <div className="mb-8 pb-8 border-b border-white/10">
                    <h3 className="font-display font-bold text-2xl mb-2 flex items-center gap-3">
                      <KeyRound className="text-sunlight-orange" /> 
                      {isGoogleUser ? "Create Password" : "Change Password"}
                    </h3>
                    <p className="text-sm text-silver-shine leading-relaxed">
                      {isGoogleUser 
                        ? "You registered via Google. Create a password here so you can also log in manually using your email." 
                        : "Ensure your account uses a strong password (minimum 8 characters) and is not used on other sites."}
                    </p>
                  </div>
                  ...
                  >
                    {!isGoogleUser && (
                      <div className="flex flex-col gap-2">
                        ...
                        <input name="currentPassword" type="password" placeholder="Enter old password" required className="w-full bg-black/30 border border-white/10 focus:border-sunlight-orange rounded-xl px-4 py-3 text-sm text-white focus:outline-none" />
                      </div>
                    )}
                    
                    <div className="flex flex-col gap-2">
                      <label className="text-sm font-bold text-white">New Password</label>
                      <input name="newPassword" type="password" placeholder="Enter new password" required minLength={6} className="w-full bg-black/30 border border-white/10 focus:border-sunlight-orange rounded-xl px-4 py-3 text-sm text-white focus:outline-none" />
                    </div>
                    
                    <div className="flex flex-col gap-2">
                      <label className="text-sm font-bold text-white">Confirm New Password</label>
                      <input name="confirmPassword" type="password" placeholder="Repeat new password" required minLength={6} className="w-full bg-black/30 border border-white/10 focus:border-sunlight-orange rounded-xl px-4 py-3 text-sm text-white focus:outline-none" />
                    </div>
                    
                    <div className="flex items-center justify-end gap-4 mt-6 pt-6 border-t border-white/10">
                      <button type="submit" className="flex items-center gap-2 bg-sunlight-orange text-blue-marine px-6 py-3 rounded-xl text-sm font-bold hover:bg-yellow-400 transition-colors shadow-lg">
                        <Save size={16} /> {isGoogleUser ? "Save Password" : "Update Password"}
                      </button>
                    </div>
                  </form>
                </>
